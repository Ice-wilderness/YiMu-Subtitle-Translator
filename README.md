...
> 目前仅针对 **英语 → 中文** 翻译进行了完整测试，其他语言对尚未验证，框架已预留多语言支持。
...
选择 VideoLingo 作为对比对象，是因为两者技术路线最接近：都使用 Whisper 进行语音转录、都通过 API 调用大模型翻译、都有术语管理机制、都关注翻译质量而非只做简单的机翻。同时它也是目前 GitHub 上同类项目中 Star 最多的（16.4k），质量保障机制较完善（翻译→反思→适配三轮翻译），可以代表当前 AI 字幕翻译的较高水平。
