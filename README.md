...
**翻译速度对比（同一台机器、同一视频、同一翻译模型）：**

| | 译幕 | VideoLingo |
|--|------|-----------|
| 翻译流程耗时 | **约 3 分钟** | 约 10 分钟 |

两者调用同一个翻译模型，译幕通过并行翻译 + 轻量检测修复，速度约为 VideoLingo 的 3 倍。

**翻译风格对比（测试视频为 AI 技术解读，涉及 GAN 架构、智能体设计等专业话题）：**
...
译幕会根据原文内容自动适配翻译风格。完整对比报告见 [docs/comparison-report.md](docs/comparison-report.md)。
